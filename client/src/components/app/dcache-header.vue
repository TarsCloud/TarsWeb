  ...
  <div class="app_index__header">
    <div class="main-width">
      ...
      <div class="logo-wrap">
        ...
        /></a>
        <a class="active" href="/dcache.html"
      ...
      >
        ...
        ></let-tab-pane>
        <let-tab-pane
          :tab="$t('header.dcache.tab2')"
          tabkey="/operation"
          :icon="opaIcon"
        ></let-tab-pane>
        <let-tab-pane
          :tab="$t('header.tab.tab3')"
          tabkey="/releasePackage"
          :icon="releaseIcon"
        ></let-tab-pane>
        <let-tab-pane
          :tab="$t('header.tab.tab4')"
          tabkey="/config"
          :icon="cacheIcon"
        ></let-tab-pane>
        <let-tab-pane
          :tab="$t('header.tab.tab5')"
          tabkey="/operationManage"
          :icon="operatorIcon"
        ></let-tab-pane>
      </let-tabs>
      <div class="language-wrap">
        <let-select v-model="locale" @change="changeLocale" :clearable="false">
          <template v-for="locale in localeMessages">
            <let-option :value="locale.localeCode">{{
              locale.localeName
            }}</let-option>
          </template>
        </let-select>
