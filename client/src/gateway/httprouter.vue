...
export default {
  ...
  methods: {
    fetchData() {
      ...
      return this.$ajax
        .getJSON("/server/api/httprouter_list", {
    ...
    fetchUpstreams() {
      return this.$ajax
        .getJSON("/server/api/upstream_list", { gatewayObj: this.gatewayObj })
        .then((data) => {
          if (data && data.rows) {
    ...
    saveItem() {
      if (this.$refs.detailForm.validate()) {
        ...
        const url = model.f_id
          ? "/server/api/update_httprouter"
          : "/server/api/add_httprouter";
    ...
    removeItem(d) {
      this.$confirm(
        ...
        .then(() => {
          const loading = this.$Loading.show();
          this.$ajax
            .postJSON("/server/api/delete_httprouter", {
              f_id: d.f_id,
              gatewayObj: this.gatewayObj,
            })
            .then(() => {
