...
export default {
  ...
  methods: {
    fetchData() {
      ...
      return this.$ajax.getJSON('/gateway/api/httprouter_list', {f_station_id: this.station.f_station_id, gatewayObj: this.gatewayObj}).then((data) => {
        loading.hide();
        this.items = data;
      }).catch((err) => {
        loading.hide();
    ...
    saveItem() {
      if (this.$refs.detailForm.validate()) {
        ...
        const url = model.f_id ? '/gateway/api/update_httprouter' : '/gateway/api/add_httprouter';
    ...
    removeItem(d) {
      this.$confirm(this.$t('gateway.delete.confirmTips'), this.$t('common.alert')).then(() => {
        const loading = this.$Loading.show();
        this.$ajax.postJSON('/gateway/api/delete_httprouter', { f_id: d.f_id, gatewayObj: this.gatewayObj }).then(() => {
          loading.hide();
          this.fetchData().then(() => {
            this.$tip.success(this.$t('common.success'));
          });
