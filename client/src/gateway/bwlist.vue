<template>
    <div class="page_operation_bwlist">
        <div class="between">
            ...
            <div>
                <let-button size="small" theme="primary" @click="loadAll">{{$t('gateway.btn.loadAll')}}</let-button>
            </div>
        </div>
...
export default {
    ...
    methods:{
        fetchData() {
            return this.$ajax.getJSON('/gateway/api/bwlist', {
        ...
        addItem(){
            if (this.$refs.detailForm.validate()) {
                ...
                const url ='/gateway/api/add_bwlist';
                const loading = this.$Loading.show();
        ...
        removeItem(d){
            this.$confirm(this.$t('gateway.delete.bwListConfirmTips'), this.$t('common.alert')).then(() => {
                const loading = this.$Loading.show();
                this.$ajax.postJSON('/gateway/api/delete_bwlist', { f_id: d.f_id, type: this.type,gatewayObj:this.gatewayObj }).then(() => {
                    loading.hide();
                    this.fetchData().then(() => {
                        this.$tip.success(this.$t('common.success'));
                    });
