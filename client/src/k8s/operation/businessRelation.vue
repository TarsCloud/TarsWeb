<template>
  <div class="page_operation_businessRelation">
    <let-button
      ...
      style="float: right"
      @click="addItem"
      >{{ $t("businessRelation.btn.add") }}</let-button
    >
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      return this.$ajax
        .getJSON("/k8s/api/query_business_relation", this.query)
        .then((data) => {
          loading.hide();
          this.items = data;
        })
    ...
    saveItem() {
      if (this.$refs.detailForm.validate()) {
        ...
        const url = model.f_id
          ? "/k8s/api/update_business_relation"
          : "/k8s/api/add_business_relation";
    ...
    removeItem(d) {
      this.$confirm(
        ...
        .then(() => {
          const loading = this.$Loading.show();
          this.$ajax
            .getJSON("/k8s/api/delete_business_relation", { f_id: d.f_id })
            .then(() => {
              loading.hide();
              this.fetchData().then(() => {
                this.$tip.success(this.$t("common.success"));
