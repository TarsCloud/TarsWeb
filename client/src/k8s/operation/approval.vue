<template>
  <div class="page_operation_approval">
    ...
    <let-table ref="table" :data="items" :empty-msg="$t('common.nodata')">
      ...
      ></let-table-column>
      <let-table-column
        :title="$t('serviceApproval.RequestMark')"
        prop="RequestMark"
      ></let-table-column>
      <let-table-column
        :title="$t('serviceApproval.RequestTime')"
        prop="RequestTime"
      ></let-table-column>
      <let-table-column :title="$t('operate.operates')" width="180px">
        <template slot-scope="scope">
          ...
          <let-table-operation @click="approvalItem(scope.row)">{{
            $t("operate.approval")
          }}</let-table-operation>
    ...
    <let-modal
      ...
      :title="
        approvalModal.isNew
          ? this.$t('deployService.title.approval')
          : this.$t('deployService.title.approval')
      "
      width="80%"
  ...
  methods: {
        ...
        .then((data) => {
          ...
          if (data.hasOwnProperty("Data")) {
            for (let i = 0; i < data.Data.length; i++) {
              data.Data[i].RequestTime = formatDate(
                data.Data[i].RequestTime,
                "YYYY-MM-DD HH:mm:ss"
              );
    ...
    saveItem() {
      if (this.$refs.detailForm.validate()) {
        ...
        this.k8sApplyModel = {
          ...
          mounts: data.Mounts.filter((item) =>
        ...
        if (
          ...
          Object.keys(data.ServerK8S.HostPort).length > 0
        ) {
    ...
    saveApprovalItem() {
      if (this.$refs.approvalForm.validate()) {
        ...
        const loading = this.$Loading.show();
        this.$ajax
          .postJSON("/k8s/api/approval_create", model)
          .then(() => {
            loading.hide();
            this.fetchData().then(() => {
              this.approvalModal.show = false;
    ...
    removeItem(d) {
      this.$confirm(
        ...
        .then(() => {
          const loading = this.$Loading.show();
          this.$ajax
            .getJSON("/k8s/api/deploy_delete", { DeployId: d.DeployId })
            .then(() => {
              loading.hide();
              this.fetchData().then(() => {
                this.$tip.success(this.$t("common.success"));
