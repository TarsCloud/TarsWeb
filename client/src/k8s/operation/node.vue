        ...
        <template v-if="!showDetail">
            <let-form inline itemWidth="200px" @submit.native.prevent="search">
                ...
                <let-form-item :label="$t('deployService.form.app')">
                    <el-select v-model="query.ServerApp" clearable
                               @change="queryAppChange" size="small" filterable>
                        <el-option v-for="item in appList" :key="item" :label="item"
                                   :value="item"></el-option>
                    </el-select>
                </let-form-item>
                <let-form-item :label="$t('deployService.form.serviceName')">
                    <el-select v-model="query.ServerName" style="width: 100%" clearable size="small">
                        <el-option v-for="item in serList" :key="item" :label="item" filterable
                                   :value="item"></el-option>
                    </el-select>
                </let-form-item>
                <let-form-item>
                    <let-button size="small" type="submit" theme="primary">{{ $t('filter.btn.searchAffinity') }}</let-button>
                </let-form-item>
                ...
                <div style="float: right;margin-top: 30px">
                    <let-button size="small" type="submit" theme="primary" style="margin-right: 10px"
                                @click="batchEditCommon" :disabled="!batchEnabled">
                        {{ $t('filter.btn.editTag') }}
                    </let-button>
                    <let-button size="small" type="submit" theme="primary" :disabled="!batchEnabled"
            ...
            <!--列表-->
            <el-table ref="table" :data="items" height="calc(100vh - 320px)" border stripe
                      @selection-change="handleSelectionChange" highlight-current-row>
                <el-table-column type="selection" width="55"></el-table-column>
                <el-table-column :label="$t('filter.title.nodeName')" prop="NodeName" width="150"></el-table-column>
                <el-table-column :label="$t('common.status')" width="150">
                    <template slot-scope="scope">
                        <el-tag size="small" effect="plain" type="success" v-if="scope.row.status=='Active'">
                            {{ scope.row.status }}
                        </el-tag>
                        <el-tag size="small" effect="plain" type="danger" v-if="scope.row.status=='Unavailable'">
                            {{ scope.row.status }}
                        </el-tag>
                    </template>
                </el-table-column>
                ...
                <el-table-column :label="$t('filter.title.nodeLabels')">
                    <template slot-scope="scope">
                        <el-tag style="margin: 2px 2px" v-for="(item, key) in scope.row.commonLabels" :key="key"
                ...
                <el-table-column :label="$t('filter.title.AbilityTag')" width="200">
                    <template slot-scope="scope">
                        <el-tag style="margin: 2px 2px" v-for="item in scope.row.NodeAbility" :key="item" effect="plain"
                ...
                <el-table-column :label="$t('filter.title.nodeAddress')" width="210">
                    <template slot-scope="scope">
                        <div v-for="item in scope.row.NodeAddress" :key="item.NodeName" style="margin: 2px 2px">
            ...
            <!--操作普通标签-->
            <el-dialog :title="$t('filter.dialog.editCommonTag')" :visible.sync="commonTagDialog.show" width="40%"
                ...
                <el-form :model="commonTagDialog.model" ref="commonForm" inline-message>
                    <el-table :data="commonTagDialog.model.commonArr" border>
                        <template slot="empty">
                            <let-table-operation @click="addCommonTag(0,commonTagDialog.model.commonArr)">
                                {{ $t('deployService.form.labelMatch.addLabel') }}
                            </let-table-operation>
                        </template>
                        <el-table-column :label="$t('filter.title.tagName')">
                            <template scope="scope">
                                <el-form-item :prop="'commonArr.' + scope.$index + '.name'"
                                              :rules="[{ validator: ((rule, value, callback) =>
                                              {validateTagName(rule, value, callback,scope.row.name)}), trigger: 'blur' }]">
                        ...
                        <el-table-column :label="$t('operate.operates')" width="120px">
                            <template slot-scope="scope">
                                <let-table-operation
                                    @click="addCommonTag(scope.$index,commonTagDialog.model.commonArr)">
                                    {{ $t('operate.add') }}
                                </let-table-operation>
                                <let-table-operation
                                    @click="delCommonTag(scope.$index,commonTagDialog.model.commonArr)">
                                    {{ $t('operate.delete') }}
                                </let-table-operation>
                            </template>
                        </el-table-column>
                ...
                <span slot="footer" class="dialog-footer">
                    <el-button @click="closeCommonTagDialog">{{ $t('common.cancel') }}</el-button>
                    <el-button type="primary" @click="saveCommonTag">{{ $t('common.submit') }}</el-button>
                </span>
            </el-dialog>

            <!--操作亲和性标签-->
            <el-dialog :title="$t('filter.dialog.editAbilityTag')" :visible.sync="abilityTagDialog.show" width="40%"
                       :before-close="closeAbilityTagDialog"
                       :close-on-click-modal='false'>
                <el-form :model="abilityTagDialog.model" ref="abilityForm" inline-message>
                    ...
                    <el-table :data="abilityTagDialog.model.abilityArr" border>
                        <template slot="empty">
                            <let-table-operation @click="addAbilityTag(0,abilityTagDialog.model.abilityArr)">
                                {{ $t('filter.tip.addAffinityGroup') }}
                            </let-table-operation>
                        </template>
                        <el-table-column :label="$t('filter.title.app')">
                            <template scope="scope">
                                <el-form-item :prop="'abilityArr.' + scope.$index + '.application'"
                                              :rules="[{ validator: ((rule, value, callback) =>
                                              {validateAppName(rule, value, callback,scope.row.application)}), trigger: 'change'}]">
                                    <el-select v-model="scope.row.application" :placeholder="$t('pub.dlg.defaultValue')"
                        ...
                        <el-table-column :label="$t('filter.title.serverName')">
                            <template scope="scope">
                                <el-select v-model="scope.row.serverName" :placeholder="$t('pub.dlg.defaultValue')"
                                           style="width: 100%" clearable filterable>
                                    <el-option v-for="item in scope.row.serverNames" :key="item" :label="item"
                                               :value="item"></el-option>
                                </el-select>
                            </template>
                        </el-table-column>
                        <el-table-column :label="$t('operate.operates')" width="120px">
                            <template slot-scope="scope">
                                <let-table-operation
                                    @click="addAbilityTag(scope.$index,abilityTagDialog.model.abilityArr)">
                                    {{ $t('operate.add') }}
                                </let-table-operation>
                                <let-table-operation
                                    @click="delAbilityTag(scope.$index,abilityTagDialog.model.abilityArr)">
                                    {{ $t('operate.delete') }}
                                </let-table-operation>
                            </template>
                        </el-table-column>
                ...
                <span slot="footer" class="dialog-footer">
                    <el-button @click="closeAbilityTagDialog">{{ $t('common.cancel') }}</el-button>
                    <el-button type="primary" @click="saveAbilityTag">{{ $t('common.submit') }}</el-button>
                </span>
            </el-dialog>
        </template>

        <!-- 展示详情 -->
        <node-detail v-if="showDetail" @closeDetail="closeDetail" :nodeDetail="nodeDetail"></node-detail>
    </div>
</template>
    ...
    methods: {
        ...
        fetchData() {
            return this.$ajax.getJSON('/k8s/api/node_select', Object.assign(this.query, {
                ...
                data.Data.forEach(item => {
                    ...
                    status == "False" ? item.status = "Unavailable" : item.status = "Active";
                })
        ...
        editCommonTag(row) {
            ...
            for (let key in row.Labels) {
                if (!key.startsWith("taf.io/node") && !key.startsWith("taf.io/ability")) {
                    this.commonTagDialog.model.commonArr.push({name: key, value: row.Labels[key]})
                }
            }
        },
        batchEditCommon() {
        ...
        saveCommonTag() {
            ...
            if (this.commonTagDialog.flag == "one") {
                ...
                queryData = {nodeName: this.commonTagDialog.model.NodeName, tags: this.commonTagDialog.model.commonArr}
            } else {
                url = "/k8s/api/batch_edit_common_tag"
                queryData = {nodeNames: nodeNames, tags: this.commonTagDialog.model.commonArr}
            }
            this.$ajax.postJSON(url, queryData).then(() => {
        ...
        async batchEditAffinity() {
            if (this.multipleSelection.length == 1) {
                ...
                this.abilityTagDialog.model.abilityArr = [{application: "", serverName: "", serverNames: ""}]
            }
        },
        saveAbilityTag() {
        ...
        addTafTag(NodeName) {
            this.$ajax.postJSON('/k8s/api/taf_ability_open', {
                NodeName,
            }).then(() => {
        ...
        delTafTag(NodeName) {
            this.$ajax.postJSON('/k8s/api/taf_ability_close', {
                NodeName,
            }).then(() => {
                this.$tip.success(this.$t('common.success'));
                this.fetchData();
        ...
        async changeApp(index, app) {
            ...
            this.$set(this.abilityTagDialog.model.abilityArr[index], "serverNames", servers);
        },
        ...
        addAbilityTag(index, obj) {
            obj.splice(index + 1, 0, {application: "", serverName: "", serverNames: []})
            this.$forceUpdate();
        },
        ...
        validateTagName(rule, value, callback, pars) { // 默认的方式新增的会不校验
            if (pars === '') {
