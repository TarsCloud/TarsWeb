  ...
  <div class="page_server_history">
    <div class="table_wrap" v-if="serverListShow">
     ...
     <let-table :data="serverList" :empty-msg="$t('common.nodata')">
        ...
        <let-table-column :title="$t('serverList.table.th.currMessage')" width="80px">
          <template slot-scope="scope">
            <let-tooltip class="tooltip" placement="top" :content="scope.row.PresentMessage || ''">
              <let-table-operation>{{scope.row.PresentMessage}}</let-table-operation>
            </let-tooltip>
          </template>
        </let-table-column>
      ...
      <let-button theme="danger" @click="getMore()">{{ $t('operate.more') }}</let-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    historySearch(c) {
      ...
      this.$ajax.getJSON('/k8s/api/server_search', {
