        ...
        <div class="left-view">
            <div class="tree_search">
                <el-input size="small"
                          ...
                          v-model="treeSearchKey" @keydown.enter.native="filterTextChange">
                </el-input>
            </div>
            <div class="tree_wrap">
                ...
                <el-tree class="left-tree"
                      ...
                      <span class="custom-tree-node" slot-scope="{ node, data }">
                            <span>
                                <i :class="data.serverType|serverTypeFilter" v-if="data.type==2"></i>
                                {{ data.name }}
                            </span>
                      </span>
                </el-tree>
        ...
        <div class="right-view">
            <div class="btabs_wrap">
                <ul ref="btabs" class="btabs" v-vscroll>
                    <li class="btabs_item" :class="{'active': homeTab === treeid }" :key="homeTab">
                        <a class="btabs_link" href="javascript:;" @click="clickBTabs($event, homeTab)"
                           style="padding-right: 20px">
                            {{ $t('home.homeTab') }}
                        </a>
                    </li>
                    <li class="btabs_item" :class="{'active': item.id === treeid}" v-for="item in BTabs" :key="item.id">
                        <a class="btabs_link" href="javascript:;" @click="clickBTabs($event, item.id)">{{ item.id }}</a>
                        <a class="btabs_close" href="javascript:;" @click="closeBTabs(item.id)">
                            {{ $t('home.close') }}</a>
                    </li>
                </ul>
                <a class="btabs_all" href="javascript:;" :title="$t('home.closeAll')" @click="closeAllBTabs">
                    {{ $t('home.closeAll') }}</a>
            </div>

            <div v-show="treeid === homeTab" class="btabs_con_home" style="height: 800px">
                <serverHistory></serverHistory>
            </div>
            <div class="btabs_con">
                <div class="btabs_con_item" v-for="item in BTabs" :key="item.id" v-if="item.id === treeid">
                    <let-tabs @click="clickTab" :activekey="item.path">
                        <let-tab-pane :tabkey="base + '/manage'" :tab="$t('header.tab.tab1')"></let-tab-pane>
                        <let-tab-pane v-if="serverData.level === 5" :tabkey="base + '/history'"
                                      :tab="$t('header.tab.tab7')"></let-tab-pane>
                    ...
                    serverData.level === 1 ? $t('index.rightView.tab.appConfig') : ''"
                                      v-if="serverData.level === 5 || serverData.level === 1"></let-tab-pane>
                        <let-tab-pane :tabkey="base + '/server-monitor'" :tab="$t('index.rightView.tab.statMonitor')"
                                      v-if="serverData.level === 5"></let-tab-pane>
                        <let-tab-pane :tabkey="base + '/property-monitor'"
                    ...
                    <router-view :is="getName(item.path)" :treeid="item.id" ref="childView"
                                 class="page_server_child"></router-view>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        getTreeData(key, type) {
            ...
            this.$nextTick(() => {
                this.loading = true;
                this.$ajax.getJSON('/k8s/api/tree', {
                    searchKey: key || '',
                    type
                }).then((res) => {
                    this.loading = false;
...
.btabs_close {
    ...
    width: 30px;
    z-index: 20;
    -webkit-transform: translateY(-50%) scale(.5) rotateZ(-180deg);
