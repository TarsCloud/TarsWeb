    ...
    <div style="height: 500px; margin-top: 10px">
      ...
      <vue-scroll :ops="ops" style="width: 100%; height: 100%">
        <let-table
          ...
          :sortable="true"
        >
          <let-table-column prop="id" title="ID" align="center" width="20%">
          </let-table-column>
          <let-table-column
            prop="timeStamp"
          ...
          <let-table-column
            ...
            width="20%"
          >
            <template slot-scope="scope"> {{ scope.row.alltime }}ms </template>
          </let-table-column>
          <let-table-column
            align="center"
...
import { getAll, unique, transTree, transTree1 } from "../../common/util";
...
export default {
  ...
  methods: {
    ...
    async sub() {
      ...
      let rest = await this.$ajax.getJSON("/server/api/detail1", this.form);
      if (rest.msg) {
        this.$Notice({
          message: rest.msg,
          type: "error",
      ...
      this.$Notice({
          message: this.$t("callChain.searchSuc"),
          type: "success",
        });
      this.options = [];
      this.flags = true;
      ...
      this.dataarr.forEach((item) => {
        ...
        item.date = this.fortime(parseInt(item.timeStamp));
        item.edges.forEach((items1) => {
          items1.etime = items1.startTimeStamp + items1.callTime;
        });
        item.edges.sort((a, b) => {
          return b.etime - a.etime;
        });
        item.timeFrame = this.fortime(parseInt(item.edges[0].etime));
        item.alltime = item.edges[0].etime - item.timeStamp;
      });
