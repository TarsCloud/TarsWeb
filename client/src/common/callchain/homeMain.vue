    ...
    <div style="height: 600px">
      <vue-scroll :ops="ops" style="width: 100%; height: 100%">
        <v-svgt
          ...
          v-bind:surenum="surenum"
           v-bind:changenum="changenum"
...
import { getAll, unique, transTree } from "../../common/util";
...
export default {
  ...
  methods: {
    async sub() {
      ...
      this.flags = true;
      this.flag2 = true;
      let rest = await this.$ajax.getJSON("/server/api/getAverage", {
        label: this.value,
        nowDate: this.today,
      });
      ...
      this.$Notice({
        message: this.$t(this.$t("callChain.searchSuc")),
        type: "success",
      });
    ...
    becomeD3(edges, nodeinfos) {
      this.$nextTick(() => {
        for (let k in edges) {
          ...
          let spans = document.querySelectorAll(`.svg${k} svg .span1`);
          for (var j in spans) {
            if (j == spans.length) {
              break;
            }
