...
import { getAll, transTree } from "../../common/util";
  ...
  var start3 = new Date().getTime();
    ...
    becomeD3(edges, nodeinfos) {
      this.$nextTick(() => {
        for (let k in edges) {
          ...
          let spans = document.querySelectorAll(`.svgF${k} svg .span1`);
          for (var j in spans) {
            if (j == spans.length) {
              break;
            }
    ...
    async ganTe(a, d, index) {
     ...
     let that =this
      ...
      var container = document.getElementById(`funccontainer${index}`);
