<template>
  <div>
    ...
    <let-table :data="tableData" :empty-msg="$t('common.nodata')" ref="pageTable">
      ...
      <let-table-column :title="$t('routerManage.version')" prop="version"></let-table-column>
      <let-table-column :title="$t('routerManage.switchStatus')" prop="switch_status">
        <template slot-scope="scope">
          <div v-for="item in switchStatus" :key="item.val" v-if="item.val === scope.row.switch_status">{{ item.label }}</div>
        </template>
      </let-table-column>
      ...
      <let-form
        ...
        <let-form-item :label="$t('routerManage.switchStatus')" required>
          <let-select
            ...
            <let-option v-for="item in switchStatus" :key="item.val" :value="item.val">{{item.label}}</let-option>
          </let-select>
        </let-form-item>
