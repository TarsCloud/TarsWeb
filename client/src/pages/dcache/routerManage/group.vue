<template>
  <div>
    ...
    <let-table :data="tableData" :empty-msg="$t('common.nodata')" ref="pageTable">
      ...
      <let-table-column :title="$t('routerManage.moduleName')" prop="module_name"></let-table-column>
      <let-table-column :title="$t('routerManage.groupName')" prop="group_name"></let-table-column>
      <let-table-column :title="$t('routerManage.accessStatus')" prop="access_status">
        <template slot-scope="scope">
          <div v-for="item in accessStatus" :key="item.val" v-if="item.val === scope.row.access_status">{{ item.label }}</div>
        </template>
      </let-table-column>
      <let-table-column :title="$t('routerManage.serverName')" prop="server_name"></let-table-column>
      <let-table-column :title="$t('routerManage.serverStatus')" prop="server_status">
        <template slot-scope="scope">
          <div v-for="item in serverStatus" :key="item.val" v-if="item.val === scope.row.server_status">{{ item.label }}</div>
        </template>
      </let-table-column>
      <let-table-column :title="$t('routerManage.pri')" prop="pri"></let-table-column>
      <let-table-column :title="$t('routerManage.sourceServerName')" prop="source_server_name"></let-table-column>
      <let-table-column :title="$t('operate.operates')" width="60px">
        <template slot-scope="scope">
          <let-table-operation @click="editEvent(scope.row.id)">{{$t('operate.update')}}</let-table-operation>
        </template>
      </let-table-column>
    </let-table>
    ...
    <let-modal
      ...
      <let-form
        v-if="dialogModal.model"
        ref="dialogForm" itemWidth="100%">
        ...
        <let-form-item :label="$t('routerManage.accessStatus')" required>
          <let-select
            ...
            <let-option v-for="item in accessStatus" :key="item.val" :value="item.val">{{item.label}}</let-option>
          </let-select>
        </let-form-item>
        ...
        <let-form-item :label="$t('routerManage.accessStatus')" required>
          <let-select
            ...
            <let-option v-for="item in serverStatus" :key="item.val" :value="item.val">{{item.label}}</let-option>
          </let-select>
        </let-form-item>
...
<style lang="postcss">
  .btn_group{display:block;margin-bottom:20px;}
  .btn_group .let-button{margin-right:10px;}
</style>