<template>
  <section style="display: inline" class="batch-publish">
    <let-button
      ...
      :disabled="disabled"
      @click="init"
      >{{ $t(`serverList.batch.${type}`) }}</let-button
    >
    <let-modal
      v-model="show"
      @on-confirm="confirm"
      ...
      :width="'1000px'"
      :title="$t(`serverList.batch.${type}`)"
    >
      <let-form>
        <let-form-item :label="$t('serverList.title.serverList')">
      ...
      <let-form class="elegant-form">
        <div class="elegant-switch">
          ...
          >
            <span slot="open">{{ $t("serverList.batch.elegantTask") }}</span>
            <span slot="close">{{ $t("serverList.batch.commonTask") }}</span>
          </let-switch>
          &nbsp;&nbsp;&nbsp;
          <div class="elegant-num" v-if="elegantChecked">
            <span class="elegant-label">{{
              $t("serverList.batch.elegantEachNum")
            }}</span>
            <let-input-number
              class="elegant-box"
              :max="20"
...
import tarsReleaseProgress from "./tarsReleaseProgress.vue";
...
export default {
  ...
  methods: {
    ...
    async addTask(serial, elegant, eachnum, items) {
      let data = await this.$ajax.postJSON("/server/api/add_task", {
        serial: serial,
        elegant: elegant,
        eachnum: eachnum || 1,
        items,
    ...
    async confirm() {
      ...
      try {
        ...
        let serial = false;
        const releaseId = await this.addTask(serial, elegant, eachnum, items);
