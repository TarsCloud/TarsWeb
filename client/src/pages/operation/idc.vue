...
<template>
  <div class="page_operation_templates">
    ...
    <el-form inline itemWidth="200px" @submit.native.prevent="search">
      <el-form-item :label="$t('idc.grid.groupName')">
        <el-input size="small" v-model="query.group_name"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    >
      ...
      ></let-table-column>
      <let-table-column
        :title="$t('idc.grid.groupName')"
        prop="group_name"
      ></let-table-column>
      <let-table-column
        :title="$t('idc.grid.groupNameCN')"
        prop="group_name_cn"
      ></let-table-column>
      <let-table-column
        :title="$t('idc.grid.ipOrder')"
        prop="ip_order"
      ></let-table-column>
      <let-table-column
        :title="$t('idc.grid.allowIpRule')"
        prop="allow_ip_rule"
      ></let-table-column>
      <let-table-column
        :title="$t('idc.grid.dennyIpRule')"
        prop="denny_ip_rule"
      ></let-table-column>
      <let-table-column
        :title="$t('idc.grid.lastuser')"
        prop="lastuser"
      ></let-table-column>
      <let-table-column
        :title="$t('idc.grid.modifyTime')"
        prop="modify_time"
      ></let-table-column>
    ...
    <let-modal
      ...
      :title="
        detailModal.isNew ? this.$t('idc.btn.add') : this.$t('idc.update.title')
      "
      width="800px"
      @on-confirm="saveItem"
    ...
    >
      <let-form
        ref="detailForm"
        v-if="detailModal.model"
        itemWidth="300px"
        columns="2"
        class="two-columns"
      >
        <let-form-item
          :label="$t('idc.grid.groupName')"
          required
          v-if="!detailModal.isNew"
        >
        ...
        <let-form-item
          :label="$t('idc.grid.groupNameCN')"
          required
          style="margin-left: 40px"
        >
          <let-input
            size="small"
            v-model="detailModal.model.group_name_cn"
            required
        ...
        <let-form-item :label="$t('idc.grid.ipOrder')" required>
          <let-select
            ...
            required
          >
            <let-option v-for="t in ipOrders" :key="t" :value="t">{{
              t
            }}</let-option>
          </let-select>
        </let-form-item>
        ...
        <let-form-item :label="$t('idc.grid.allowIpRule')">
          <let-select v-model="detailModal.model.allowList" required multiple>
            <let-option v-for="t in allowList" :key="t" :value="t">{{
              t
            }}</let-option>
          </let-select>
        </let-form-item>
        <let-form-item
          :label="$t('idc.grid.dennyIpRule')"
          style="margin-left: 40px"
        >
          <let-select v-model="detailModal.model.dennyList" required multiple>
            <let-option v-for="t in dennyList" :key="t" :value="t">{{
              t
            }}</let-option>
          </let-select>
        </let-form-item>
        <let-form-item itemWidth="242px">
          <let-input
            v-model="addIp.addAllow"
            :placeholder="$t('idc.update.addIp')"
            size="small"
          ></let-input>
        </let-form-item>
        <let-button theme="primary" size="small" @click="addAllowIp">{{
          $t("operate.add")
        }}</let-button>
        <let-form-item itemWidth="242px" style="margin-left: 40px">
          <let-input
            v-model="addIp.addDenny"
            :placeholder="$t('idc.update.addIp')"
            size="small"
          ></let-input>
        </let-form-item>
        <let-button theme="primary" size="small" @click="addDennyIp">{{
          $t("operate.add")
        }}</let-button>
      </let-form>
    </let-modal>
...
export default {
  ...
  data() {
    return {
      ...
      ipOrders: ["denny_allow", "allow_denny"],
      allowList: [],
      dennyList: [],
      addIp: {
        addAllow: "",
  ...
  methods: {
    fetchData() {
      ...
      return this.$ajax
        .getJSON("/server/api/query_idc", this.query)
        .then((data) => {
          loading.hide();
          this.items = data;
        })
    ...
    removeItem(row) {
      this.$confirm(
        this.$t("idc.delete.confirmTips"),
        this.$t("common.alert")
      ).then(() => {
        const loading = this.$Loading.show();
        this.$ajax
          .getJSON("/server/api/delete_idc", row)
          .then(() => {
            loading.hide();
            this.fetchData().then(() => {
    ...
    addAllowIp() {
      if (this.validIp(this.addIp.addAllow)) {
        //已经添加
        if (
          this.allowList.includes(this.addIp.addAllow) &&
    ...
    addDennyIp() {
      if (this.validIp(this.addIp.addDenny)) {
        //已经添加
        if (
          this.dennyList.includes(this.addIp.addDenny) &&
