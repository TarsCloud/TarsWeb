<template>
  <div class="page_operation_application">
    ...
    <let-table ref="table" :data="items" :empty-msg="$t('common.nodata')">
      ...
      ></let-table-column>
      <let-table-column
        :title="$t('application.form.person')"
        prop="f_create_person"
      ></let-table-column>
      <let-table-column
        :title="$t('application.form.time')"
        prop="f_create_time"
      ></let-table-column>
    ...
    <let-modal
      ...
      :title="$t('application.view.title')"
      width="800px"
    >
    ...
    <let-modal
      ...
      :title="
        detailModal.isNew
          ? this.$t('application.add.title')
          : this.$t('application.update.title')
      "
      width="800px"
      @on-confirm="saveItem"
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      return this.$ajax
        .getJSON("/server/api/query_application", this.query)
        .then((data) => {
          loading.hide();
          this.items = data;
        })
    ...
    saveItem() {
      if (this.$refs.detailForm.validate()) {
        ...
        const url = model.id
          ? "/server/api/update_application"
          : "/server/api/add_application";
    ...
    removeItem(d) {
      this.$confirm(
        this.$t("application.delete.confirmTips"),
        this.$t("common.alert")
      )
        .then(() => {
          const loading = this.$Loading.show();
          this.$ajax
            .getJSON("/server/api/delete_application", { f_id: d.f_id })
            .then(() => {
              loading.hide();
              this.fetchData().then(() => {
                this.$tip.success(this.$t("common.success"));
